...
def ingest_taxi_lookup(user, password, host, port, db, table_name, csv_file):
    print(table_name, csv_file)
    
    engine = connect_db(user, password, host, port, db)
    ...
    raw_data_iter = pd.read_csv(csv_file, iterator=True, chunksize=100000) # returns a generator useful for batching
...
def ingest_callable(user, password, host, port, db, table_name, csv_file):
    ...
    raw_data_df.lpep_pickup_datetime = pd.to_datetime(raw_data_df.lpep_pickup_datetime)
    raw_data_df.lpep_dropoff_datetime = pd.to_datetime(raw_data_df.lpep_dropoff_datetime)
    headers = raw_data_df.head(n=0)
